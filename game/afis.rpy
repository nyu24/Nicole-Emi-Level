...
# ---------------------------------------------------------------------------------------
    ...
    set_scores(print_name="print_3", scores=var_scores)
...
label show_results:
    ...
    if prints[imported_print].scores[current_print][0]:
        ...
        if imported_print == "print_1":
            ...
            n normal2 "At the very least this checks out with the Alastor's friends' testimony."
            n normal2 "I'll get the police to question them more about the bag."
        $ print_imported = False
        $ imported_print = ""
        jump lab_nonina
    else:
